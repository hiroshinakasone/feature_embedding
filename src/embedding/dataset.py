...
class MovieLensUserDataset(Dataset):
    def __init__(self, rating_file_path):
        ...
        df = pd.read_csv(
            rating_file_path, usecols=["movieId", "userId", "rating"], engine="python"
        )
        ...
        self._m1_indices, self._m2_indices = np.where(
            (cosine_sim >= 0.8) | (cosine_sim <= -0.8)
        )
        self._labels = np.array(
            [
                1 if cosine_sim[tuple(idx)] >= 0.8 else -1
                for idx in zip(self._m1_indices, self._m2_indices)
            ],
            dtype=np.int8,
        )
